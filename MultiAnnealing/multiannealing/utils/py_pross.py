...
class TypedList:
    ...
    def __copy__(self):
        ...
        have = lambda k: k in new.__dict__
...
class molMol(TypedList):
    ...
    def num_chain(self):
        """Returns the number of chains in the molMol list."""
        return len(self)

    def num_res(self):
        """Returns the number of residues in each of the chains in the molMol list."""
        nr = 0
        for chain in self: nr = nr + len(chain)
        return nr

    def num_atoms(self):
        """Returns the number of atoms in each of the chains in the molMol list."""
        na = 0
        for chain in self: na = na + chain.num_atoms()
        return na

    def atoms(self):
        """Returns a list of the atoms in the molMol list."""
        ret = []
        Append = ret.append
    ...
    def chains_with_name(self, *names):
        """Returns each of the chains named with specified names."""
        chains = []
        for chain in self.elements:
    ...
    def residues_with_name(self, *names):
        """Returns each of the residues named with specified names."""
        residues = []
        for chain in self.elements:
            for res in chain.elements:
    ...
    def atoms_with_name(self, *names):
        """Returns a list of atoms named with  specified names."""
        ret = []
        Append = ret.append
        for chain in self.elements:
            for res in chain.elements:
    ...
    def atom_coordinates(self, *names):
        """Returns a python list with the coordinates of specified atoms."""
        coords = []
        if len(names) == 0:
            for chain in self.elements:
                for res in chain.elements:
    ...
    def assign_radii(self):
        """Assign the radius of each of the chains in the molMol TypedList."""
        for chain in self:
            chain.assign_radii()

    def delete_water(self):
        """Delete the water molecules from chains in the molMol TypedList."""
        for chain in self:
            chain.delete_water()

    def preserve_chain_hetero(self):
        """Executes the preserve_chain_hetero method on chains in the molMol TypedList."""
        for i in self.indices():
            self[i].preserve_chain_hetero()

    def delete_hetero(self):
        """Executes the delete_hetero method on chains in the molMol TypedList."""
        for i in self.reverse_indices():
...
class Atom3d:
    """A TypedList class for atoms with 3-D coordinates."""
    _is_an_atom3d = 1
    ...
    def __repr__(self):
        """Formats atom coordinates into a printable format."""
        return "Atom3d(%s, %s, %s)" % (self.x, self.y, self.z)
    __str__ = __repr__

    def coords(self):
        """Returns the 3-D coordinates of an atom."""
        if HAVE_NUMPY:
    ...
    def set_coords(self, x, y=None, z=None):
        """Sets the 3-D coordinates of an atom."""
        if y is None:
    ...
    def clone(self):
        """Creates a copy of the Atom3d list."""
        new = Atom3d(0., 0., 0.)
...
#       return self.__class__(0, 0, 0, cnf)
    ...
    def distance(self, other, sqrt=math.sqrt):
        """Calculate the distance between an atom and a specified other."""
...
def get_sequences(file):
    """Find the residue sequences and return them in a dictionary."""
    if file[-3:] == '.gz':
...
def unpack_pdb_line(line):
    """Gather atom information from the pdb lines, convert them to a usable format."""
...
def is_protein(chain):
    """Determines if residues in a chain are proteins."""
    for res in chain.elements:
...
def _read_chain(f, as_protein=0, as_rna=0, as_dna=0, unpack=unpack_pdb_line,
    ...
    """Reads chains of elements from a pdb file and returns them as a Chain type."""
...
class Protein(molChain):
    """A TypedList containing Proteins, set by `_is_a_protein`."""
    _is_a_protein = 1
...
#            return 999.99
    ...
    def backbone_atoms(self, *ids):
        """If the atom name matches any in bb_atoms, make a new residue and add to a chain."""
        if not len(ids):
            ids = range(len(self))
...
class PDBFile:
    """Contains methods for reading chains from a PDB File."""
...
def run(file,mcode):
    """Reads a pdb file and iterates through each chain, and groups residues
    Together, and then finally, writes the residues to angfile.
    """
