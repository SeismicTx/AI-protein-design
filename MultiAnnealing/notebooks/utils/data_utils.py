...
class MSA_processing:
    def __init__(self,
        ...
        preprocess_MSA=True,
        threshold_sequence_frac_gaps=0.3,
...
class CSV_processing:
    def __init__(self, csv_location, unsup_data):
        self.sup_df = pd.read_csv(csv_location, index_col=0)
        self.encoding = self.sup_df.values
        self.headers = self.sup_df.index.values
        try:
#             print(unsup_data.seq_name_to_sequence.keys())
#             print(len(unsup_data.seq_name_to_sequence.keys()))
#             print(self.headers)
#             print(len(self.headers))
            assert (np.array([k[1:] for k in unsup_data.seq_name_to_sequence.keys()]) == self.headers).sum() == unsup_data.one_hot_encoding.shape[0]
        except AssertionError:
            assert (np.array([k[1:].replace('/','_') for k in unsup_data.seq_name_to_sequence.keys()]) == self.headers).sum() == unsup_data.one_hot_encoding.shape[0]
        print('labels for data: ' + str(self.encoding.shape))